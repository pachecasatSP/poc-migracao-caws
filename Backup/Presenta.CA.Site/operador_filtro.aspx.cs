...
{
    public partial class operador_filtro : System.Web.UI.Page
    {
        public int _id = 0;
            ...
            try
            {
                ...
                if (!this.IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        {
                            txtNome.Text = clsCA.SessionRetornar(clsCA._PARAM_RET_DSC);                        
                            _voltar = true;
                        }
                        if (this.txtNome.Text.Length > 0 || _voltar)
                            Filtrar();
                    }
                    //
                }
        ...
        protected void ucCAToolbar_Search(object sender, EventArgs e)
        {
            try
            {
                ...
                clsCA.SessionInserir(clsCA._PARAM_RET_DSC, txtNome.Text);
                //
                Filtrar();
            }
            catch (Exception ex)
        ...
        {
            Server.Transfer("operador_cadastro.aspx?" + clsCA._PARAM_NOVO + "=1");
        }

        private void Filtrar()
        {
            clsOperador _clsPrincipal = new clsOperador();
            gvFiltro.DataSource = _clsPrincipal.RetornarLista(null, null, txtNome.Text, null);
