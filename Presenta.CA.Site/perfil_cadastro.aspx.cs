...
{
    public partial class perfil_cadastro : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request.QueryString["codigo"] != null)
                    _idperfil = Convert.ToInt32(Request.QueryString["codigo"]);
                ...
                if (!this.IsPostBack)
                {
                    ucSituacaoPerfil.CarregarDados("SituacaoPerfil");
        ...
        private void CarregarDados()
        {
            ...
            clsPerfil _clsPrincipal = new clsPerfil();
            List<clsEntPerfil> _clsListaEntidade = _clsPrincipal.Retornar(_idperfil);
            _Erros = _clsPrincipal.Erros;
            if (_Erros == null || _Erros.Length == 0)
            {
                foreach (clsEntPerfil _clsEntidade in _clsListaEntidade)
                {
                    hfCodigo.Value = _clsEntidade.idperfil.ToString();
                    txtDsPerfil.Text = _clsEntidade.dsperfil;
                    ucSituacaoPerfil.Selecionar(_clsEntidade.stperfil);
        ...
        protected void ucCAToolbar_Save(object sender, EventArgs e)
        {
            ...
            {
                clsPerfil _clsPrincipal = new clsPerfil();
                clsEntPerfil _clsEntidade = new clsEntPerfil();

                if (!_Insercao)
                    _clsEntidade.idperfil = Convert.ToInt32(hfCodigo.Value);

                _clsEntidade.dsperfil = txtDsPerfil.Text;
                _clsEntidade.stperfil = Convert.ToInt32(ucSituacaoPerfil.cboSituacao.SelectedValue.ToString());
                ...
                if (_Erros == null || _Erros.Length == 0)
                {
                    Response.Redirect("perfil_filtro.aspx?" + clsCA._PARAM_SAVE + "=1");
                }
            }
            catch (Exception ex)
            {
        ...
        protected void ucCAToolbar_Delete(object sender, EventArgs e)
        {
            try
            {
                ...
                _Erros = _clsPrincipal.Erros;
                if (_Erros == null || _Erros.Length == 0)
                    Response.Redirect("perfil_filtro.aspx?" + clsCA._PARAM_DELETE + "=1");
            }
            catch (Exception ex)
            {
                _Erros = ex.Source + " - " + ex.Message;
        ...
        {
            Response.Redirect("perfil_filtro.aspx?" + clsCA._PARAM_VOLTAR + "=1");
        }
