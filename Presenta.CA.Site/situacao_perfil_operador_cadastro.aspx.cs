...
{
    public partial class situacao_perfil_operador_cadastro : System.Web.UI.Page
    {
        private bool _Insercao;
        public string _Erros = string.Empty;
        private int? stperfiloperador;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request.QueryString["codigo"] != null)
                    stperfiloperador = int.Parse(Request.QueryString["codigo"]);
        ...
        private void CarregarDados()
        {
            ...
            clsSituacaoPerfilOperador _clsPrincipal = new clsSituacaoPerfilOperador();
            List<clsEntSituacaoPerfilOperador> _clsListaEntidade = _clsPrincipal.Retornar(stperfiloperador);
            _Erros = _clsPrincipal.Erros;
            if (_Erros == null || _Erros.Length == 0)
            {
                foreach (clsEntSituacaoPerfilOperador _clsEntidade in _clsListaEntidade)
                {
                    hfCodigo.Value = _clsEntidade.stperfiloperador.ToString();
                    txtDsSituacao.Text = _clsEntidade.dsperfiloperador.ToString();
        ...
        protected void ucCAToolbar_Save(object sender, EventArgs e)
        {
            ...
            {
                clsSituacaoPerfilOperador _clsPrincipal = new clsSituacaoPerfilOperador();
                clsEntSituacaoPerfilOperador _clsEntidade = new clsEntSituacaoPerfilOperador();

                if (_Insercao == false)
                    _clsEntidade.stperfiloperador = Convert.ToInt32(hfCodigo.Value);

                _clsEntidade.dsperfiloperador = txtDsSituacao.Text;
                ...
                if (_Erros == null || _Erros.Length == 0)
                {
                    Response.Redirect("situacao_perfil_operador_filtro.aspx?" + clsCA._PARAM_SAVE + "=1");
                }
            }
            catch (Exception ex)
            {
        ...
        protected void ucCAToolbar_Delete(object sender, EventArgs e)
        {
            try
            {
                ...
                _Erros = _clsPrincipal.Erros;
                if (_Erros == null || _Erros.Length == 0)
                    Response.Redirect("situacao_perfil_operador_filtro.aspx?" + clsCA._PARAM_DELETE + "=1");
            }
            catch (Exception ex)
            {
                _Erros = ex.Source + " - " + ex.Message;
        ...
        {
            Response.Redirect("situacao_perfil_operador_filtro.aspx?" + clsCA._PARAM_VOLTAR + "=1");
        }
